...
// LAST LEVEL CACHE
#define LLC_SET NUM_CPUS*8192*8
#define LLC_WAY 32
...
class CACHE : public MEMORY {
  public:
    ...
    void handle_fill(),
         ...
         handle_read(),
         handle_prefetch();
    int back_invalidate(uint8_t c_type,int addr,uint32_t fill_cpu,int fill_level,uint32_t instruction_id);
    uint8_t higher_level_dirty(uint64_t address,uint64_t &data,uint32_t fill_cpu);
    void back_invalidate(uint64_t address,uint32_t cpu);
    void send_for_exclusive(uint64_t address,int fill_cpu,PACKET p,uint8_t &do_fill);
